...
# SIG_P20
df_sig = pd.read_csv(
    base_path / "intermediaire" / "input_verif" / "SIG_P20.CSV",
    sep=';',
    decimal=','
)
...
df_clients = pd.read_csv(
    base_path / "intermediaire" / "output" / "pharmacies_clients_65plus_HUFF_EXACT_IRIS.csv"
)

# Pharmacies avec scores et type_zone
df_pharmacies = pd.read_csv(
    base_path / "intermediaire" / "output" / "pharmacies_avec_scores_attractivite.csv"
)

print(f"   SIG_P20               : {len(df_sig_agg):,} pharmacies")
print(f"   Clients 65+           : {len(df_clients):,} pharmacies")
print(f"   Pharmacies scores     : {len(df_pharmacies):,} pharmacies")
...
df_merged = df_merged.merge(
    df_pharmacies[['id_pharmacie', 'type_zone', 'departement', 'ca_total', 'ca_ethique',
                   'ca_conseil', 'score_attractivite_65plus_composite']],
    on='id_pharmacie',
    how='left'
)
...
print("\n   GLOBAL :")
corr_global = df_merged[['UN', 'CAHT', 'clients_65_plus_total', 'ca_total']].corr()
...
if df_merged['type_zone'].notna().sum() > 0:
    for zone in ['rural', 'urbain']:
        ...
        if len(df_zone) > 10:
            print(f"\n   {zone.upper()} :")
            corr_zone = df_zone[['UN', 'CAHT', 'clients_65_plus_total', 'ca_total']].corr()
...
try:
    ...
    graph_path = base_path / "intermediaire" / "output" / "correlation_avancee_graphs.png"
...
output_path = base_path / "intermediaire" / "output" / "correlation_avancee_complete.csv"
...
summary_path = base_path / "intermediaire" / "output" / "summary_correlations_par_categorie.csv"
