...
from config_prep import INPUT_FILES, OUTPUT_FILES, INPUT_DIR, OUTPUT_DIR

# Importer la configuration des catégories
from a_finess_categories import FINESS_CATEGORIES, get_category, get_type

# Fichiers depuis config
FINESS_FILE = INPUT_FILES['finess']
OUTPUT_FILE = OUTPUT_FILES['finess_merged']
...
def main():
    ...
    # Top 20 catégories
    print(f"\n{'='*80}")
    print(f"TOP 20 CATÉGORIES")
    print(f"{'='*80}")

    top_categories = df_final.groupby('categorie_simplifiee').agg({
        'nofinesset': 'count'
    })
    top_categories.columns = ['Nombre']
    top_categories = top_categories.sort_values('Nombre', ascending=False).head(20)

    for idx, row in top_categories.iterrows():
        print(f"  {idx:40s} | N={row['Nombre']:>6,}")
