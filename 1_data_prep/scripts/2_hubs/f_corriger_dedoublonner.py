...
CACHE_DIR = BASE_DIR / "1_data_prep" / "cache"
...
def load_hubs():
    ...
    hubs_file = BASE_DIR / "1_data_prep" / "output" / "HUBS_unified.csv"
...
def display_summary(df):
    ...
    for idx, (cat, count) in enumerate(df['categorie'].value_counts().head(15).items(), 1):
        pct = (count / total) * 100
        print(f"   {idx:>2}. {cat:<35} : {count:>7,} ({pct:>4.1f}%)")
    
    print(f"\n📁 FICHIER CRÉÉ:")
    print(f"   • 1_data_prep/output/HUBS_unified_final.csv")
