...
from config_prep import INPUT_DIR, CACHE_DIR, RPPS_GEOCODE_DIR, OUTPUT_FILES, GEOCODING_WORKERS

# Supprimer les warnings
warnings.filterwarnings('ignore')

# Configuration des chemins depuis config
DOWNLOADS_DIR = Path.home() / "Downloads"  # Alternative pour RPPS
OUTPUT_DIR = RPPS_GEOCODE_DIR
...
def load_and_geocode_rpps_parallel(max_to_geocode=None):
    ...
    # Cache
    cache_file = CACHE_DIR / "RPPS_merged.csv"
    ...
    try:
        ...
        for idx, row in df_filtered.iterrows():
            ...
            row_data = {
                ...
                # 'bonus_attractivite': config.get('bonus', 0.10),  # Supprimé - non utilisé
                'source': 'RPPS',
            }
...
def save_results(df):
    ...
    output_file = OUTPUT_DIR / "RPPS_merged.csv"
...
def display_summary(df):
    ...
    # Score de confiance géocodage
    scores = df[df['geo_score'] > 0]['geo_score']
    if len(scores) > 0:
        print(f"\n🎯 Score confiance moyen: {scores.mean():.3f}")
    
    print(f"\n📁 Fichier créé:")
    print(f"   • 1_data_prep/output/rpps_geocode/RPPS_merged.csv")
