...
from config_prep import INPUT_DIR as CONFIG_INPUT_DIR, ISOCHRONES_DIR, CACHE_DIR, OUTPUT_FILES

# Supprimer les warnings non critiques
warnings.filterwarnings('ignore', category=UserWarning)
warnings.filterwarnings('ignore', category=FutureWarning)

# Configuration des chemins depuis config
INPUT_DIR = CONFIG_INPUT_DIR  # Les fichiers pharmacies sont dans 1_data_prep/input
OUTPUT_DIR = ISOCHRONES_DIR
CACHE_DIR_ISOCHRONES = CACHE_DIR / "isochrones"  # Sous-dossier cache isochrones
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, help='Dossier de sortie (défaut: 1_data_prep/output/isochrones/)')
        ...
        if args.input:
            if Path(args.input).is_absolute():
                ...
                if not input_file.exists():
                    # Fallback vers 1_data_prep/input
                    input_file = BASE_DIR / "1_data_prep" / "input" / args.input
        else:
            # Chercher automatiquement pharmacies_final.csv en priorité
            possible_files = [
                INPUT_DIR / "pharmacies_final.csv",
                BASE_DIR / "1_data_prep" / "input" / "pharmacies_final.csv", 
                INPUT_DIR / "pharmacies.csv",
                BASE_DIR / "1_data_prep" / "input" / "pharmacies.csv"
            ]
            ...
            if not input_file:
                raise FileNotFoundError("Aucun fichier pharmacies trouvé dans 1_data_prep/input/data_cleaning/ ou 1_data_prep/input/")
